...
ParserResult<Expr> Parser::parseExprPrimary(Diag<> ID, bool isExprBasic) {
  ...
  case tok::pattern_literal: // single-quote pattern e.g. '(\d+)-(\d+)-(\d+)'
    return parseExprPatternLiteral();
...
ParserResult<Expr> Parser::parseExprPatternLiteral() {
  ...
  SourceLoc Loc = Tok.getLoc();
  unsigned Length = Tok.getLength() - 1; // number of characters until end quote
  ...
  // The subexpression depends on knowing what type we are going to
  // resolve to, so we have to do tapeExpr->setSubExpr() in 
  ...
  auto builderVarDecl =
      new (Context) VarDecl(/*IsStatic=*/false, VarDecl::Introducer::Var,
                            /*NameLoc=*/SourceLoc(),
                            // FIXME: is dollar interpolation okay here?
                            // I think it should be since we 
                            // are creating a new scope, but not exactly
                            // clear what the implications are 
                            // for name collision
                            Context.Id_dollarInterpolation, CurDeclContext);

  builderVarDecl->setImplicit(true); // Variable is implicitly generated by the compiler
  builderVarDecl->setUserAccessible(false);
  setLocalDiscriminator(builderVarDecl); // TODO: what does this do?
  
  bodyStatements.push_back(builderVarDecl);

  auto builderVarRef = new (Context) DeclRefExpr(builderVarDecl, 
                                                 DeclNameLoc(), 
                                                 /*Implicit=*/true);
